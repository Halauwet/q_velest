...
# sys.path.append('/mnt/d/Linux/PyVelest/tools/utility')
...
def velest2gmt(model_nm, init_pha, final_pha, main_out, plot=False, gmtplot=False):
    """
    ...
    if len(main_out) == len(init_pha) and len(init_pha) == len(final_pha):
        ...
        for i in range(len(main_out)):
            ...
            velestdata, init_data, final_data, ids = ReadMainVelest(main_out[i])
            ...
            intv_dly = np.round((max(sta_dly_arr) - min(sta_dly_arr)) / 3, decimals=2)

            min_dly = np.round(min(sta_dly_arr), decimals=2)
            max_dly = np.round(min_dly + intv_dly * 3, decimals=2)

            # Write variable plot batch (EDIT USING F STRINGS)
            if flag_iset:
                fvar.write('@echo off\n')
                fvar.write('set ps1=Hypo_adj-iSet-' + str(iset) + '.ps\n')
                fvar.write('set ps2=Model_adj-iSet-' + str(iset) + '.ps\n')
                fvar.write('set ps3=EQ_location_adj-iSet-' + str(iset) + '.ps\n')
                fvar.write('set iset=Set-' + str(iset) + '\n')
                ...
                fvar.write('set init_cat=%~dp0' + model_nm + '-CatalogInitial-iSet_' + str(iset) + '.dat' + '\n')
                fvar.write('set fnal_cat=%~dp0' + model_nm + '-CatalogFinal-iSet_' + str(iset) + '.dat' + '\n')
                fvar.write('set hypo_adj=%~dp0' + model_nm + '-HypoAdjustment-iSet_' + str(iset) + '.dat' + '\n')
                fvar.write('set init_stat=%~dp0' + model_nm + '-StationStatistic-iSet_' + str(i) + '.dat' + '\n')
                fvar.write('set sts_stat=%~dp0' + model_nm + '-StationStatistic-iSet_' + str(iset) + '.dat' + '\n')
                fvar.write('set init_mod=%~dp0' + model_nm + '-ModelInitial-iSet_' + str(iset) + '.dat' + '\n')

                if i == 0:
                    fvar.write('set init_lgn=Initial Model' + '\n')
                else:
                    fvar.write('set init_lgn=Initial Set-' + str(iset) + '\n')

                if len(velestdata) >= 5:
                    for k in range(len(velestdata)):
                        fvar.write('set mod_adj' + str(k + 1) + '=%~dp0' + model_nm + '-ModelAdjustment-iSet_'
                                   + str(iset) + '_' + str(k + 1) + '.dat' + '\n')
                else:
                    for k in range(5):
                        fvar.write('set mod_adj' + str(k + 1) + '=%~dp0' + model_nm + '-ModelAdjustment-iSet_'
                                   + str(iset) + '_' + str(k + 1) + '.dat' + '\n')
                ...
                fvar.write('set R_dmod=' + str(min_dvel) + '/' + str(max_dvel) + '/' + str(min(dep) - 2) + '/' + str(
                    max(dep) + 2) + '\n')
                fvar.write('set R_nhyp=' + str(min(dep) - 1) + '/' + str(max(dep) + 2) + '/' + str(-5) + '/' + str(
                    max_nhyp) + '\n')
                fvar.write('set R_nhyp2=' + str(-5) + '/' + str(max_nhyp) + '/' + str(min(dep) - 1) + '/' + str(
                    max(dep) + 2) + '\n')
            ...
            if len(velestdata) > 5:
                print('Sub iteration more than 5, periksa script plot')
            if len(velestdata) < 5:
                print('\nSub iteration less than 5, iteration stop...')
    ...
    intv_dly = np.round((max(sta_dly) - min(sta_dly)) / 3, decimals=2)

    min_dly = np.round(min(sta_dly), decimals=2)
    max_dly = np.round(min_dly + intv_dly * 3, decimals=2)
    ...
    for k in range(len(itr_all)):
        ...
        if k == 0:
            ...
            if allitt <= 20:
                fvel_leg.write('echo 1 ' + str(len(itr_all)) + ' > leg.tmp\n')
                fvel_leg.write('echo 3 ' + str(len(itr_all)) + ' >> leg.tmp\n')
                fvel_leg.write('echo 3.5 ' + str(len(itr_all)) +
                               ' %init_lgn% | pstext -J -R -F+f11p,Helvetica,black+jLM -O -K >> %ps2%\n')
                fvel_leg.write('psxy leg.tmp -J -R -W1.4,black,- -t30 -N -O -K >> %ps2%\n')
            else:
                fvel_leg.write('echo 1 21  > leg.tmp\n')
                fvel_leg.write('echo 3 21  >> leg.tmp\n')
                fvel_leg.write('echo 3.5 21 Initial Model | pstext -J -R '
            ...
            fvarall.write('set mod_adj' + str(k) + '=' + model_nm + '-ModelAdjustment-iter' + str(k) + '.dat' + '\n')
            transp = np.ceil(k / (len(clr) - 1)) * 15 + 15
            fvel_mod.write('gawk "{print $2, $1}" %mod_adj' + str(k) +
                           '% | psxy -J -R -W1,' + clr[cycle] + ' -O -K -t' + str(transp) + ' >> %ps2%\n')
            fvel_adj.write('gawk "{print $3, $1}" %mod_adj' + str(k) +
                           '% | psxy -J -R -SB0.22b0 -G' + clr[cycle] + ' -t' + str(transp) + ' -O -K >> %ps2%\n')
            if allitt <= 20:
                fvel_leg.write('echo 3 ' + str(len(itr_all) - k) +
                               ' | psxy -J -R -SB0.2b1 -G' + clr[cycle] + ' -t30 -O -K >> %ps2%\n')
                fvel_leg.write('echo 3.5 ' + str(len(itr_all) - k) + ' Iterasi ' +
                               str(k) + ' | pstext -J -R -F+f11p,Helvetica,black+jLM -O -K >> %ps2%\n')
            else:
                fvel_leg.write('echo 3 ' + str(21 - k * 20 / allitt) +
                               ' | psxy -J -R -SB0.2b1 -G' + clr[cycle] + ' -t30 -O -K >> %ps2%\n')
                fvel_leg.write('echo 3.5 ' + str(21 - k * 20 / allitt) + ' Iterasi ' +
                               str(k) + ' | pstext -J -R -F+f9p,Helvetica,black+jLM -O -K >> %ps2%\n')
...
def damping_test(model=None, which_dmp=None, min_damp=1, max_damp=900, num_damp=6):
    """
    ...
    if model is None:
        ...
        if len(inpmod) == 0:
            ...
            print(f'\nList model:\n\n')
            for i, m in zip(len(mod_nm), mod_nm):
                print(f'{i+1} {mod_nm}')
            mod_number = input(f'\nSelect model number:\n\n')
    ...
    set_phafl = os.path.join('input', f'phase_{mod_nm}.cnv')
    set_outmn = os.path.join(out_dir, f'mainprint_testdamp{mod_nm}.out')
    set_outph = os.path.join(out_dir, f'finalhypo_testdamp{mod_nm}.cnv')
    set_outst = os.path.join(out_dir, f'stacorrect_testdamp{mod_nm}.out')
    # set_outmd = os.path.join(out_dir, f'outmodel_testdamp{mod_nm}.mod')
    ...
    # plot model and data variance
    # select best trade of variance
...
def adjust_layer(vel, dep, step=2, dev=0.05, plot=True, flag_v=False):
    ...
    for i in range(0, len(new_vel), step):
        ...
        for j in range(len(new_vel), star, -step):

            stop = j

            lay = new_vel[star:stop]
            ...
            if dvdt < dev and mndt - dev <= vel[j - 1] <= mndt + dev:
                ...
                for k in range(star, stop):
                    new_vel[k] = round(statistics.mean(lay), 3)

                if flag_v:
                    print(f'\n adjust depth {dep[star]} - {dep[stop - 1]}:\n  {lay}\n  to\n  {new_vel[star:stop]}')
...
def Run_VelestSet(itrmax=5, invratio=1, time_out=120):
    ...
    for mod, num, md_nm in zip(inpmod, range(len(inpmod)), mod_nm):
        ...
        while j < 1:
            ...
            if iset == 1:
                set_model = [mod]  # .replace(os.sep, '/')]
                set_stafl = [os.path.join('input', 'station.dat')]  # .replace(os.sep, '/')]
                set_phafl = [os.path.join('input', 'phase.cnv')]  # .replace(os.sep, '/')]
                set_outmn = [
                    os.path.join(out_dir, f'mainprint{str(iset).zfill(2)}_{md_nm}.out')]  # .replace(os.sep, '/')]
                set_outph = [
                    os.path.join(out_dir, f'finalhypo{str(iset).zfill(2)}_{md_nm}.cnv')]  # .replace(os.sep, '/')]
                set_outst = [
                    os.path.join(out_dir, f'stacorrect{str(iset).zfill(2)}_{md_nm}.out')]  # .replace(os.sep, '/')]
                set_outmd = [
                    os.path.join(out_dir, f'outmodel{str(iset).zfill(2)}_{md_nm}.mod')]  # .replace(os.sep, '/')]

            else:
                set_model.append(set_outmd[i - 1])
                set_stafl.append(set_outst[i - 1])
                set_phafl.append(set_outph[i - 1])
                set_outmn.append(os.path.join(out_dir,
                                              f'mainprint{str(iset).zfill(2)}_{md_nm}.out'))  # .replace(os.sep, '/'))
                set_outph.append(os.path.join(out_dir,
                                              f'finalhypo{str(iset).zfill(2)}_{md_nm}.cnv'))  # .replace(os.sep, '/'))
                set_outst.append(os.path.join(out_dir,
                                              f'stacorrect{str(iset).zfill(2)}_{md_nm}.out'))  # .replace(os.sep, '/'))
                set_outmd.append(os.path.join(out_dir,
                                              f'outmodel{str(iset).zfill(2)}_{md_nm}.mod'))  # .replace(os.sep, '/'))

            cmnout = open('velest.cmn', 'w')

            with open(os.path.join('input', 'baseset.cmn')) as f:
